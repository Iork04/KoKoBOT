...
[占卜] 随机选取牌阵进行占卜。
[塔罗牌] 得到单张塔罗牌回应。
[开启/启用/关闭/禁用群聊转发] 开启或关闭全局群聊转发(SUPERUSER)。'''

__plugin_meta__ = PluginMetadata(name="塔罗牌",
                                 description="塔罗牌！魔法占卜🔮",
                                 usage=__tarot_usages__,
                                 extra={
                                     "author":
                                     "KafCoppelia <k740677208@gmail.com>",
                                     "version": __tarot_version__
                                 })

divine = on_command(cmd="占卜", priority=7)
tarot = on_command(cmd="塔罗牌", priority=7)
chain_reply_switch = on_regex(pattern=r"^(开启|启用|关闭|禁用)群聊转发(模式)?$",
