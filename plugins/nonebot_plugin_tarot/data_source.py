...
def chain_reply(
    bot: Bot, chain: List[Dict[str, Union[str, Dict[str,
                                                    Union[str,
                                                          MessageSegment]]]]],
    msg: MessageSegment
...
def pick_theme() -> str:
    ...
    if len(sub_themes_dir) > 0:
        return random.choice(
            list(
                set(sub_themes_dir).union(tarot_config.tarot_official_themes)))
...
def pick_sub_types(theme: str) -> List[str]:
    ...
    sub_types: List[str] = [
        f.name for f in (tarot_config.tarot_path / theme).iterdir()
...
class Tarot:
    ...
    async def divine(self, bot: Bot, matcher: Matcher,
    ...
    def _random_cards(
            self,
            all_cards: Dict[str, Dict[str, Dict[str, Union[str, Dict[str,
                                                                     str]]]]],
            theme: str,
            num: int = 1) -> List[Dict[str, Union[str, Dict[str, str]]]]:
        '''
        ...
        subset: Dict[str, Dict[str, Union[str, Dict[str, str]]]] = {
            k: v
            for k, v in all_cards.items() if v.get("type") in sub_types
        }
    ...
    async def _get_text_and_image(
        self, theme: str, card_info: Dict[str, Union[str, Dict[str, str]]]
