...
[今日老婆] 抽取今日老婆。
[换老婆] 换一个老婆。
[刷新今日老婆] 清除群聊今日老婆数据(GROUP_ADMIN)。
[开启/关闭今日老婆] 开启/关闭功能(GROUP_ADMIN)。
[设置换老婆次数 ?] 设置群聊可以换老婆的次数(GROUP_ADMIN)。"""
...
# 刷新所在群全部记录
today_waifu_refresh = on_regex(rf"^\s*(刷新|重置)(?P<name>{PatternStr})\s*$",
...
async def _(event: GroupMessageEvent, val: Dict[str, Any] = RegexDict()):
    ...
    group_record: Dict[str, Union[bool, Dict[str,
                                             Dict[str,
                                                  int]]]] = get_group_record(
                                                      gid)  # 获取本群记录字典
...
async def _(event: GroupMessageEvent, times: Dict[str, Any] = RegexDict()):
    ...
    try:
        ...
        return
    gid = str(event.group_id)
    group_record: Dict[str, Union[int, Dict[str,
                                            Dict[str,
                                                 int]]]] = get_group_record(
                                                     gid)  # 获取本群记录字典
...
async def _(bot: Bot, event: GroupMessageEvent):
    ...
    group_record: Dict[str, Union[int, bool,
                                  Dict[str, Dict[str,
                                                 int]]]] = get_group_record(
                                                     gid)  # 获取本群记录字典
    limit_times: int = group_record.setdefault('limit_times',
                                               default_limit_times)
    allow_change_waifu: bool = group_record.setdefault(
        'allow_change_waifu', default_allow_change_waifu)
    ...
    group_today_record: Dict[str, Dict[str,
                                       int]] = group_record[today]  # 获取本群今日字典
    ...
    save_group_record(gid, group_record)
    try:
        member_info = await bot.get_group_member_info(group_id=gid,
                                                      user_id=new_waifu_id)
    except ActionFailed:
        # 群员已经退群情况
        member_info = {}
    message: Message = await construct_change_waifu_msg(
        member_info, new_waifu_id, int(bot.self_id), old_times, limit_times)
...
async def _(bot: Bot, event: GroupMessageEvent):
    ...
    try:
        ...
        # 群员已经退群情况
        member_info = {}
    message: Message = await construct_waifu_msg(member_info, waifu_id,
                                                 int(bot.self_id), is_first)
