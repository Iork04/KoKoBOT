...
__in_history_usage__ = """
[历史上的今天] 发送历史上的今天。
[历史上的今天 状态/设置/关闭] 配置历史上的今天推送。"""
__plugin_meta__ = PluginMetadata(
    name="历史上的今天",
    description="发送每日历史上的今天",
    usage=__in_history_usage__,
    type="application",
    homepage=
...
async def handle_time(event: MessageEvent,
    ...
    if match and match[1] and match[2]:
        if isinstance(event, GroupMessageEvent):
            calendar_subscribe("g_{}".format(event.group_id), int(match[1]),
                               int(match[2]))
        else:
            calendar_subscribe("f_{}".format(event.user_id), int(match[1]),
                               int(match[2]))
