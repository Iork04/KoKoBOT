...
__plugin_usage__ = """
[装弹 (子弹数) (金额) (at)] 金币默认为200，若不指定决斗对象则群友都可参加。

[接受对决/拒绝决斗] 拒绝或者接受此次对决。

[开枪 (子弹数)] 轮流开枪，根据子弹数量连开N枪械(未指定则为1)，30秒未开枪另一方可使用‘结算’命令结束对决并胜利。

[结算] 当某一方30秒未开枪，可使用该命令强行结束对决并胜利。

[我的战绩] 查看自己的战绩。

[金币/胜场/败场/欧洲人/慈善家排行] 查看各种排行。
...
russian = on_command("俄罗斯轮盘",
                     aliases={"装弹", "俄罗斯转盘"},
                     permission=GROUP,
                     priority=5,
                     block=True)

accept = on_command("接受对决",
                    aliases={"接受决斗", "接受挑战"},
                    permission=GROUP,
                    priority=5,
                    block=True)

refuse = on_command("拒绝对决",
                    aliases={"拒绝决斗", "拒绝挑战"},
                    permission=GROUP,
                    priority=5,
                    block=True)
...
async def get_bullet_num(event: GroupMessageEvent,
                         state: T_State,
...
async def _(bot: Bot,
            event: GroupMessageEvent,
            state: T_State = Depends(get_bullet_num)):
    ...
    if at_:
        ...
        at_player_name = (at_player_name["card"] if at_player_name["card"] else
                          at_player_name["nickname"])
        msg = (
            f"{player1_name} 向 {MessageSegment.at(at_)} 发起了决斗！请 {at_player_name} 在30秒内回"
            f"复‘接受对决’ or ‘拒绝对决’，超时此次决斗作废！")
    else:
        at_ = 0
        msg = "若30秒内无人接受挑战则此次对决作废【首次游玩请发送 ’help 俄罗斯轮盘‘ 来查看命令】"

    _msg = russian_manager.ready_game(event, msg, player1_name, at_, money,
                                      bullet_num)
...
async def _(event: GroupMessageEvent, state: T_State):
    msg = await russian_manager.rank(state["_prefix"]["raw_command"],
                                     event.group_id)
...
async def _():
    ...
    await _reset_gold.finish("每日轮盘签到重置成功...")
