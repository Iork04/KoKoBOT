...
reminder_expire_time = (plugin_config.reminder_expire_time * 24 *
                        3600 if plugin_config.reminder_expire_time else 3 *
                        24 * 3600)
...
async def _(bot: Bot, event: MessageEvent):
    ...
    if not message_list:
        await who_at_me.finish(
            MessageSegment.reply(event.message_id) + "目前还没有人@您噢！")
    if is_group:
...
async def _(event: MessageEvent):
    if isinstance(event, GroupMessageEvent):
        MainTable.delete().where(
            (MainTable.target_id == event.user_id)
            & (MainTable.group_id == event.group_id)).execute()
        await clear_db.finish("已经清除您在本群的被艾特记录！")
    else:
        MainTable.delete().where(
            MainTable.target_id == event.user_id).execute()
        await clear_db.finish("已经清除您所有的被艾特记录！")


clear_db_all = on_command("清除全部数据库",
                          aliases={"clear_all", "db_clear --purge"},
